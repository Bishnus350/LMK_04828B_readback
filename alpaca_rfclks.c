...
int spi_get_lmk04828_config(spi_dev_t *dev, uint32_t* regbuf) {
  ...
  for (int i=0; i<REGISTER_COUNT; i++, lmk_cd++) {
  ...
  printf("\n");
  printf("LMK register readbacks are R386, R387, R392: 0x%06x, 0x%06x, 0x%06x", lmk_config_data[0], lmk_config_data[1], lmk_config_data[2]);
    // Implement logic based on th ereadback values
    printf("\n");
    uint8_t pll1_dld_status = (lmk_config_data[0] & (1<<1)) ? 1 :0; //0x182 bit 4
    uint8_t pll2_dld_status = (lmk_config_data[1] & (1<<1)) ? 1 :0; //0x183 bit 1
    uint8_t led_status = (lmk_config_data[2] & (1<<4)) ? 0 :1; //0x188 bit 4


    printf ("LED is %s\n", led_status ? "lit": "not lit");
    printf ("PLL1 DLD is %s\n", pll1_dld_status ? "set": "not set");
    printf ("PLL2 DLD is %s\n", pll2_dld_status ? "set": "not set");

    if (!led_status && pll1_dld_status && pll2_dld_status){
            printf ("LED is lit while PLL1 DLD and PLL2 DLD are both set.\n") ;
    }
 ...
 */
#ifdef I2C_COM_BUS
int get_pll_config( uint32_t* regbuf) {
#else
